...
/datum/nanite_program/dermal_button/set_extra_setting(user, setting)
	...
	if(setting == NES_BUTTON_NAME)
		var/new_button_name = input(user, "Choose the name for the button.", NES_BUTTON_NAME, button_name, MAX_NAME_LEN)
		if(!new_button_name)
			return
...
/datum/nanite_program/dermal_button/proc/press()
	if(activated)
		host_mob.visible_message("<span class='notice'>[host_mob] presses a button on forearm.</span>",
