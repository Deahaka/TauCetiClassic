...
/datum/nanite_program/self_scan/on_trigger(comm_message)
	...
	switch(NS.get_value())
		if("Medical")
			var/data = health_analyze(host_mob, host_mob, TRUE, FALSE)
			var/datum/browser/popup = new(host_mob, "[host_mob.name]_scan_report", "[host_mob.name]'s scan results", 400, 400, ntheme = CSS_THEME_LIGHT)
			popup.set_content(data)
			popup.open()
		if("Nanite")
			if(!(SEND_SIGNAL(host_mob, COMSIG_NANITE_SCAN, host_mob, FALSE) & COMPONENT_NANITES_DETECTED))
				to_chat(host_mob, "<span class='info'>No nanites detected in the subject.</span>")
...
/datum/nanite_program/dermal_button/proc/press()
	if(activated)
		host_mob.visible_message("<span class='notice'>[host_mob] presses a button on [host_mob] forearm.",
								"<span class='notice'>You press the nanite button on your forearm.")
