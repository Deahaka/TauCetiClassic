...
/datum/nanite_program/triggered/comm/speech/set_extra_setting(user, setting)
	if(setting == "Sentence")
		var/new_sentence = input(user, "Choose the sentence that the host will be forced to say.", "Sentence", sentence, MAX_MESSAGE_LEN)
		if(!new_sentence)
			return
...
/datum/nanite_program/triggered/comm/voice/set_extra_setting(user, setting)
	if(setting == "Message")
		var/new_message = input(user, "Choose the message sent to the host.", "Message", message, MAX_MESSAGE_LEN)
		if(!new_message)
			return
		message = new_message
	if(setting == "Comm Code")
...
/datum/nanite_program/triggered/comm/hallucination/set_extra_setting(user, setting)
	...
	if(setting == "Hallucination Type")
		...
		switch(hal_type_choice)
			...
			if("Message")
				hal_type = "Message"
				var/hal_chat = input(user, "Choose the message the host will hear, or leave empty for random messages.", "Message", hal_details, MAX_MESSAGE_LEN)
				if(hal_chat)
...
/datum/nanite_program/good_mood/set_extra_setting(user, setting)
	if(setting == "Mood Message")
		var/new_message = input(user, "Choose the message visible on the mood effect.", "Message", message, MAX_NAME_LEN)
		if(!new_message)
			return
		message = new_message
