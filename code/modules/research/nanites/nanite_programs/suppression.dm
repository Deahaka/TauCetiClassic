...
/datum/nanite_program/comm/speech/set_extra_setting(user, setting)
	if(setting == NES_SENTENCE)
		var/new_sentence = input(user, "Choose the sentence that the host will be forced to say.", NES_SENTENCE, sentence, MAX_MESSAGE_LEN)
		if(!new_sentence)
			return
...
/datum/nanite_program/comm/voice/set_extra_setting(user, setting)
	if(setting == NES_MESSAGE)
		var/new_message = input(user, "Choose the message sent to the host.", NES_MESSAGE, message, MAX_MESSAGE_LEN)
		if(!new_message)
			return
		message = new_message
	if(setting == NES_COMM_CODE)
...
/*
	if(comm_message && (hal_type != NES_MESSAGE)) / via comm remote, but not set to a message hallucination
		return
	...
	if(setting == NES_HALLUCINATION_TYPE)
		...
		switch(hal_type_choice)
			...
			if(NES_MESSAGE)
				...
				var/hal_chat = input(user, "Choose the message the host will hear, or leave empty for random messages.", NES_MESSAGE, hal_details, MAX_MESSAGE_LEN)
				if(hal_chat)
...
/datum/nanite_program/good_mood/set_extra_setting(user, setting)
	if(setting == NES_MOOD_MESSAGE)
		var/new_message = input(user, "Choose the message visible on the mood effect.", NES_MESSAGE, message, MAX_NAME_LEN)
		if(!new_message)
			return
		message = new_message
