...
/datum/nanite_program/sensor/voice/set_extra_setting(user, setting)
	...
	if(setting == NES_SENTENCE)
		var/new_sentence = input(user, "Choose the sentence that triggers the sensor.", NES_SENTENCE, sentence, MAX_MESSAGE_LEN)
		if(!new_sentence)
			return
...
/datum/nanite_program/sensor/race
	...
	extra_settings = list(NES_SENT_CODE,"Race","Mode")
	var/race_type = HUMAN
	var/mode = "Is"
	var/list/static/allowed_species = list(
		...
		IPC = /datum/species/machine,
		DIONA = /datum/species/diona,
		PODMAN = /datum/species/diona/podman,
		SLIME = /datum/species/slime
	)
