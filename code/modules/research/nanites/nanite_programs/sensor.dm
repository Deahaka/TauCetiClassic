...
/datum/nanite_program/sensor/crit/check_event()
	if(host_mob.stat == UNCONSCIOUS && host_mob.health <= 0)
		if(!spent)
			spent = TRUE
			return TRUE
		return FALSE
...
/datum/nanite_program/sensor/voice/set_extra_setting(user, setting)
	if(setting == "Sent Code")
		...
		sent_code = clamp(round(new_code, 1), 1, 9999)
	if(setting == "Sentence")
		var/new_sentence = input(user, "Choose the sentence that triggers the sensor.", "Sentence", sentence, MAX_MESSAGE_LEN)
		if(!new_sentence)
			return
...
/datum/nanite_program/sensor/cloud_sync
	name = "Cloud Sync Sensor"
	desc = "The nanites receive a signal when the cloud sync is enabled or disabled."
	can_rule = TRUE
	extra_settings = list("Sent Code","Type")
	var/last_cloud = FALSE
	var/check_type = "On Switch"

/datum/nanite_program/sensor/cloud_sync/on_add(datum/component/nanites/_nanites)
	..()
	last_cloud = nanites.cloud_active

/datum/nanite_program/sensor/cloud_sync/set_extra_setting(user, setting)
	if(setting == "Sent Code")
		var/new_code = input(user, "Set the sent code (1-9999):", name, null) as null|num
		if(isnull(new_code))
			return
		sent_code = clamp(round(new_code, 1), 1, 9999)
	if(setting == "Type")
		var/new_type = input("When should the sensor trigger?", name) as null|anything in list("On Disable","On Enable","On Switch")
		if(!new_type)
			return
		check_type = new_type

/datum/nanite_program/sensor/cloud_sync/get_extra_setting(setting)
	if(setting == "Sent Code")
		return sent_code
	if(setting == "Type")
		return check_type

/datum/nanite_program/sensor/cloud_sync/copy_extra_settings_to(datum/nanite_program/sensor/cloud_sync/target)
	target.sent_code = sent_code
	target.check_type = check_type

/datum/nanite_program/sensor/cloud_sync/check_event()
	if(nanites.cloud_active == last_cloud)
		return FALSE
	var/previous_cloud = last_cloud
	last_cloud = nanites.cloud_active
	switch(check_type) //We already verified that they're different so we can save some checks
		if("On Disable")
			if(previous_cloud == TRUE)
				return TRUE
		if("On Enable")
			if(previous_cloud == FALSE)
				return TRUE
		if("On Switch")
			return TRUE
	return FALSE

/datum/nanite_program/sensor/cloud_sync/make_rule(datum/nanite_program/target)
	if(check_type == "On Switch") //Can't make a rule out of this
		return
	var/datum/nanite_rule/cloud_sync/rule = new(target)
