	...
	switch(action)
		...
		if("save")
			if(locked)
				return
			//todo sanitize?
			var/code_name = input(usr, "Set the setting name", "Set Name", null , 15)
			if(!code_name)
				return
			var/new_save = list()
...
/obj/item/nanite_remote/comm/tgui_interact(mob/user, datum/tgui/ui)
	SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "nanite_comm_remote", name) //, 420, 800, master_ui, state)
		ui.open()
...
/obj/item/nanite_remote/comm/tgui_act(action, list/params)
	...
	switch(action)
		...
		if("set_message")
			if(locked)
				return
			var/new_message = input(usr, "Set the message (Max 300 characters):", "Set Message", null , 300)
			if(!new_message)
				return
